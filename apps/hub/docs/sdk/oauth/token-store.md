...
Purpose: Browser token store is transient by default.

## Owns

- Browser token store is transient by default.
- Server token store is adapter-based.
- SDK must not store client secrets.
...
- Keep this file focused on token store only.
