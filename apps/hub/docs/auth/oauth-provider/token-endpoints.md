...
Purpose: Token endpoint exchanges code and verifier for access token.

## Owns

- Token endpoint exchanges code and verifier for access token.
- Refresh token requires offline_access.
- Revocation and introspection must be rate-limited.
...
- Keep this file focused on token endpoints only.
