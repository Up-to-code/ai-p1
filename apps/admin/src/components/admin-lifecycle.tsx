...
function AdminNestedSections({ sections, locale }: { sections: AdminDetailResponse["sections"]; locale: Locale }) {
  const sectionCopy: Record<string, { title: string; description: string }> = {
    ...
    business: {
      ...
      description: text(locale, "Projects, assets, clients, tasks, bookings, and media owned by this organization.", "المشاريع والوحدات والعملاء والمهام والحجوزات والوسائط الخاصة بهذه المؤسسة."),
    },
    audit: {
...
function humanizeAction(action: string, locale: Locale) {
  const en: Record<string, string> = {
    ...
    "client asset link": "Client linked to asset",
    "client asset unlink": "Client unlinked from asset",
    "asset media attach": "Asset media attached",
  ...
  const ar: Record<string, string> = {
    ...
    "client asset link": "تم ربط عميل بأصل",
    "client asset unlink": "تم فصل عميل عن أصل",
    "asset media attach": "تمت إضافة وسائط للأصل",
