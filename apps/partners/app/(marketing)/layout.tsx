    ...
    <>
      <header className="sticky top-0 z-50 flex h-[69px] w-full items-center justify-between border-b border-border bg-white/88 px-4 backdrop-blur-lg dark:bg-card/90 sm:px-6">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <Link href="/security" className="hidden rounded-[7px] px-2 py-1.5 text-sm font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground md:inline-flex">Security</Link>
          <ThemeToggle />
          <Link href="/signin" className="rounded-[7px] px-3 py-2 text-sm font-semibold text-foreground transition-colors hover:bg-muted">Sign in</Link>
          <Link href="/signup" className="rounded-[7px] bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground transition-colors hover:bg-[#6b90e6]">Create app</Link>
        </div>
      </header>
      {children}
    </>
