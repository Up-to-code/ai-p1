...
Use OAuth 2.1 endpoints directly. Do not depend on an `@anan/auth-sdk` package unless your Qentrah contact has provided a private build.
...
export function AuthorizeWithQentrahButton() {
  return <a href="/api/anan/oauth/start">Authorize with Qentrah</a>;
}
```
