...
Purpose: Use HMAC verification with constant-time compare where available.

## Owns

- Use HMAC verification with constant-time compare where available.
- Sign raw body, timestamp, and event ID.
- Do not verify parsed JSON only.
...
- Keep this file focused on signature verification only.
