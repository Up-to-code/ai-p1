...
Purpose: API key record stores hash, prefix, scopes, status, organization, platform, expiry, and audit metadata.

## Owns

- API key record stores hash, prefix, scopes, status, organization, platform, expiry, and audit metadata.
- No raw key field.
...
- Keep this file focused on api key record only.
