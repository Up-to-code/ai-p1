...
Purpose: Every sensitive action records actor, organization, action, resource, reason, timestamp, and request ID.

## Owns

- Every sensitive action records actor, organization, action, resource, reason, timestamp, and request ID.
- Audit logs are immutable in intent.
- Audit reads are permission checked.
...
- Keep this file focused on auditability only.
