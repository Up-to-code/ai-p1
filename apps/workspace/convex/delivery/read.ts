...
export const engagementDetail = query({
  ...
  returns: v.union(v.object({
    ...
    approvals: v.array(approvalValidator), changeOrders: v.array(changeOrderValidator), concerns: v.array(concernValidator),
  }), v.null()),
  handler: async (ctx, args) => {
    const access = await resolveDeliveryAccess(ctx, args.organizationId);
    ...
    const [projects, deliverables, approvals, changeOrders, concerns] = await Promise.all([
      ctx.db.query("engagementProjects").withIndex("by_engagement_project", (q) => q.eq("organizationId", args.organizationId).eq("engagementId", engagement._id)).collect(),
    ...
    return {
      engagement,
      projects: projects.filter((record) => !record.deletedAt),
