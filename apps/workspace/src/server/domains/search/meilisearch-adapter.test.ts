...
describe("Meilisearch search Adapter", () => {
  it("keeps credentials server-side and applies tenant, principal, sensitivity, type, and locale filters", async () => {
    ...
    const result = await adapter.search({ organizationId: "org_1", text: "alpha", principalKeys: ["user:user_1"], resourceTypes: ["task"], locales: ["ar"], sensitivity: ["standard"], limit: 10 });
