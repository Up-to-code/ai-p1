...
## Purpose
Owns backend contracts and future Hono implementation units for the teams bounded context.
...
When teams endpoints are implemented, start in routing, delegate to handlers, validate input through validators/validation, check access through access/policies, then call services.
