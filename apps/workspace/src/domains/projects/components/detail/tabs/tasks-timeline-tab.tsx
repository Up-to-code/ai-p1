...
import type { ViewItem, ViewType } from "@/components/shared/view-system/types";
import type { TaskStatus, TaskPriority, TaskRecord } from "@/domains/tasks/tasks.types";

const FALLBACK_PROJECT_TABS: ViewItem[] = [
  { id: "project-board", type: "board", label: "Board" },
  { id: "project-table", type: "table", label: "Table" },
  { id: "project-timeline", type: "timeline", label: "Timeline" },
];
        ...
        return (
          <div key={col.value} className="space-y-2">
            <div className="flex items-center gap-2 px-1 mb-3">
              ...
              <span className="text-[10px] font-bold text-muted-foreground/60 ml-auto">{columnTasks.length}</span>
            </div>
            <div className="space-y-1.5 min-h-[200px] rounded-xl border border-dashed border-border/50 p-2">
              {columnTasks.map((task) => (
                <div key={task.id} className="group rounded-lg border border-border bg-card p-3 hover:border-primary/20 transition-colors cursor-pointer">
                  ...
                  <div className="flex items-center gap-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {statusColumns.filter((s) => s.value !== task.status).slice(0, 2).map((s) => (
                      <button
                        key={s.value}
                        onClick={(e) => { e.stopPropagation(); onStatusChange(task, s.value); }}
...
function ProjectTimelineView({ tasks, project }: { tasks: TaskRecord[]; project: Project }) {
  const ganttTasks = useMemo(() => {
    ...
    return tasks
      ...
      .map((t) => {
        ...
        const startMs = dueMs - 3 * 86400000;
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      <div className="flex items-center border-b border-border bg-muted/30 px-4 py-2">
        <div className="w-48 shrink-0 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Task</div>
        ...
        {ganttTasks.map((task: any) => (
          <div key={task.id} className="flex items-center px-4 py-2.5 hover:bg-muted/10 transition-colors group">
            <div className="w-48 shrink-0 min-w-0">
              ...
              {task.dueDate && <p className="text-[10px] text-muted-foreground mt-0.5">{new Date(task.dueDate).toLocaleDateString()}</p>}
            </div>
            <div className="flex-1 relative h-5">
              <div
                className={cn(
                  "absolute h-4 rounded-sm top-0.5 transition-all duration-300",
                  task.status === "done" ? "bg-emerald-500/60" : task.status === "inProgress" ? "bg-blue-500/60" : task.status === "waiting" ? "bg-amber-500/60" : "bg-muted-foreground/30",
                )}
                style={{ left: `${task.leftPct}%`, width: `${Math.max(task.widthPct, 2)}%` }}
              />
            </div>
          </div>
        ))}
...
export function TasksTimelineTab({ project, organizationId, spaceId }: TasksTimelineTabProps) {
  ...
  const {
    ...
    reorderTabs,
    renameTab,
  } = useViewTabs({ scope: tasksScope, defaultTabs });
    ...
    <div className="space-y-6">
      ...
      <ViewTabsContent
        ...
        renderTab={(tab) => {
          switch (tab.type) {
            case "board":
              return <ProjectBoardView tasks={tasks} onStatusChange={handleStatusChange} />;
            case "table":
              return <TaskTableView tasks={tasks} />;
            case "timeline":
              return <ProjectTimelineView tasks={tasks} project={project} />;
            default:
              return <div className="p-8 text-center text-muted-foreground text-sm">{tab.type} view coming soon</div>;
          }
        }}
      />
