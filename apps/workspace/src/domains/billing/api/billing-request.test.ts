...
describe("billing request wrappers", () => {
  ...
  it("uses shared encoded organization paths for billing requests", async () => {
    ...
    await createCheckoutRequest({ organizationId: "org 1", planId: "good_monthly", returnUrl: "https://example.com/billing" });
    ...
    expect(fetcher).toHaveBeenNthCalledWith(3, "/api/v1/organizations/org%201/billing/checkout", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ planId: "good_monthly", returnUrl: "https://example.com/billing" }),
    });
