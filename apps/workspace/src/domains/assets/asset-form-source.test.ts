...
describe("asset form source", () => {
  const source = readSource("src/domains/assets/components/assets-screens.tsx");
  const createPage = readSource("src/app/[locale]/(app)/assets/create/page.tsx");
  const editPage = readSource("src/app/[locale]/(app)/assets/[id]/edit/page.tsx");

  it("uses one shared form component for create and edit", () => {
    expect(createPage).toContain("<AssetFormScreen />");
    expect(editPage).toContain("<AssetFormScreen id={id} />");
  });

  it("uses a custom searchable project picker instead of a native select", () => {
    expect(source).toContain("function AssetProjectPicker");
  ...
  it("writes the selected project id and name into the asset form state", () => {
  ...
  it("does not render the asset form side preview", () => {
  ...
  it("keeps asset form controls inline instead of nested card grids", () => {
    expect(source).toContain("function AssetInlineChoice");
    expect(source).toContain("AssetHelpLabel");
