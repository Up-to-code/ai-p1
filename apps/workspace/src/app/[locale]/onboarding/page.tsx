...
import { createOrganizationInvitation, updateAuthOrganization } from "@/domains/organization/api";
...
export default function OnboardingPage() {
  ...
  const [manage, setManage] = useState<(typeof manageOptions)[number] | "">("it");
  ...
  async function finish() {
    ...
    try {
      ...
      await Promise.allSettled(
        invites.map((email) => createOrganizationInvitation(activeOrg.id, { email, role: "member" })),
      );
      router.push("/ws");
    } catch (caught) {
  ...
  if (!session?.user || !activeOrg) {
    return (
      <div className="flex min-h-[70svh] w-full max-w-lg flex-col items-center justify-center text-center">
        ...
        <Link className="mt-8 inline-flex h-12 items-center justify-center rounded-2xl bg-foreground px-6 text-sm font-bold text-background hover:bg-foreground/90" href="/choose-org">
  ...
  return (
    <main className="min-h-svh w-full bg-[radial-gradient(circle_at_18%_12%,rgba(255,70,110,0.18),transparent_28%),radial-gradient(circle_at_82%_6%,rgba(58,130,255,0.14),transparent_24%),#050607] px-4 py-6 text-white sm:px-6">
      <section className="mx-auto flex min-h-[calc(100svh-3rem)] max-w-5xl items-center justify-center">
        <div className="relative min-h-[620px] w-full overflow-hidden rounded-[10px] border border-white/8 bg-[#0d0e10] shadow-2xl shadow-black/50">
          <div className="pointer-events-none absolute inset-x-0 top-0 h-36 bg-[linear-gradient(100deg,rgba(255,91,52,0.24),rgba(183,52,180,0.18),rgba(32,95,185,0.18),transparent)]" />
          <button
            type="button"
            aria-label={t("close")}
            onClick={() => router.push("/ws")}
            className="absolute right-5 top-5 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-white/10 text-white/70 transition hover:bg-white/15 hover:text-white"
          >
            <X className="h-4 w-4" />
          </button>

          <div className="relative z-10 flex min-h-[620px] flex-col px-7 py-6 sm:px-9">
              ...
              <div className="w-full">
                  ...
                  <StepBlock title={t("purposeTitle")}>
                    <PillGrid>
                      {purposeOptions.map((option) => (
                        <ChoicePill key={option} selected={purpose === option} onClick={() => setPurpose(option)}>
                          {t(`purpose.${option}`)}
                        </ChoicePill>
                      ))}
                    </PillGrid>
                  ...
                  <StepBlock title={t("manageTitle")}>
                    <PillGrid>
                      {manageOptions.map((option) => (
                        <ChoicePill key={option} selected={manage === option} onClick={() => setManage(option)}>
                          {t(`manage.${option}`)}
                        </ChoicePill>
                      ))}
                    </PillGrid>
                ...
                {currentStep === "invite" ? (
                  <StepBlock title={t("inviteTitle")}>
                    ...
                    <p className="mt-2 text-xs font-semibold text-white/30">{t("inviteEnter")}</p>
                    {invites.length > 0 ? (
                      <div className="mt-4 flex flex-wrap gap-2">
                        {invites.map((email) => (
                          <button
                            type="button"
                            key={email}
                            onClick={() => setInvites((current) => current.filter((item) => item !== email))}
                        ...
                        return (
                          ...
                          >
                            <span className="flex min-w-0 items-center gap-2">
                              ...
                              <span className="truncate">{t(`features.${feature.id}`)}</span>
                            </span>
                            {selected ? <Check className="h-4 w-4" /> : <Circle className="h-4 w-4 opacity-40" />}
                          </button>
                        );
                      })}
                    </div>
                ...
                {currentStep === "name" ? (
                  <StepBlock title={t("nameTitle")}>
                    ...
                    />
                    <p className="mt-4 text-xs font-semibold text-white/30">{t("nameHint")}</p>
                  </StepBlock>
                ) : null}
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between">
                ...
                {currentStep === "name" ? (
                  <Button type="button" onClick={() => void finish()} disabled={!canGoNext || isFinishing} className="h-10 rounded-lg bg-white px-5 text-sm font-bold text-black hover:bg-white/90">
                    {isFinishing ? <Loader2 className="h-4 w-4 animate-spin" /> : t("finish")}
                    {!isFinishing ? <Check className="h-4 w-4" /> : null}
                  </Button>
                ) : (
                  <Button type="button" onClick={next} disabled={!canGoNext} className="h-10 rounded-lg bg-white px-5 text-sm font-bold text-black hover:bg-white/90 disabled:opacity-40">
                    {t("next")}
...
function StepBlock({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="max-w-4xl">
      <h1 className="mb-6 text-2xl font-bold tracking-0 text-white sm:text-3xl">{title}</h1>
      {children}
    </div>
  );
}
...
function ChoicePill({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
