        ...
        <nav className={cn(
          ...
          <div className="flex items-center gap-2">
            <div className="hidden h-10 items-center gap-1 rounded-full bg-zinc-950/5 p-1 dark:bg-white/10 sm:flex">
              <ThemeToggle className="h-8 w-8 rounded-full border-none bg-card/80 shadow-none hover:bg-card" />
            ...
            <button
              ...
              className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[var(--q-border)] bg-white/80 text-[var(--q-text-primary)] transition hover:bg-[var(--q-card)] md:hidden dark:border-white/10 dark:bg-white/10 dark:text-white dark:hover:bg-white/15"
          ...
          <div className="absolute inset-x-0 top-full mt-2 rounded-[24px] border border-[var(--q-border)] bg-background p-3 shadow-[0_22px_80px_rgba(15,23,42,0.16)] md:hidden dark:border-white/10">
            <nav className="grid gap-1" aria-label={t("home")}>
              {mobileNavItems.map((item) => (
                <Link
                  className="flex h-10 w-full items-center rounded-2xl px-4 text-sm font-semibold text-[var(--q-text-primary)] transition hover:bg-[var(--q-card-hover)] focus:bg-[var(--q-card-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 dark:text-zinc-100 dark:hover:bg-white/10 dark:focus:bg-white/10"
                  href={item.href}
                  key={item.key}
