...
function AgentCard({ agent, isAr }: { agent: AgentItem; isAr: boolean }) {
  ...
  return (
    <div className="flex items-center gap-3.5 rounded-2xl border border-border bg-[var(--q-card)] p-4 shadow-sm dark:border-border dark:bg-zinc-950/40 hover:border-border dark:hover:border-border transition duration-300 w-full">
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[var(--q-border)] bg-muted dark:border-border dark:bg-card">
        <Image
          alt=""
          width={20}
          height={20}
      ...
      <div className="min-w-0 text-start">
        ...
        <p className="mt-1 text-[11px] font-medium text-[var(--q-text-muted)] dark:text-muted-foreground leading-normal truncate">{descText}</p>
      </div>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center" dir={isAr ? "rtl" : "ltr"}>
          <div className="flex flex-col space-y-6 text-start rtl:text-right ltr:text-left max-w-xl mx-auto lg:mx-0">
            <h2 className="text-3xl font-extrabold leading-tight tracking-tight text-foreground md:text-4xl lg:text-5xl rtl:leading-[1.25]">
              {labels.title}
            </h2>
            <p className="text-base font-semibold leading-relaxed text-secondary-foreground md:text-lg rtl:leading-[1.7]">
              {labels.description}
            </p>
          </div>

          <div className="w-full flex justify-center">
            <div className="relative h-[380px] w-full max-w-[350px] overflow-hidden rounded-3xl border border-border bg-muted/20 p-4 [mask-image:linear-gradient(to_bottom,transparent,black_12%,black_88%,transparent)]">
              <div
                className={cn(
