    ...
    <section className="px-6 py-20 md:py-32">
      <div className="mx-auto max-w-5xl">
        <div className="mb-16 text-center">
          ...
          <h2 className="mt-6 text-4xl font-bold tracking-tight text-[var(--q-text-primary)] md:text-6xl">
            {t("title")}
          </h2>
        </div>
        ...
        <div className="relative space-y-12 before:absolute before:inset-0 before:ml-5 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-zinc-200 before:to-transparent dark:before:via-white/10 md:before:mx-auto md:before:ml-auto">
          {items.map((item, index) => (
            <div key={item.title} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
              ...
              {/* Content */}
              <div className="ml-10 w-[calc(100%-4rem)] rounded-[2rem] border border-zinc-100 bg-[var(--q-card-hover)] p-6 transition-all duration-500 hover:border-[var(--q-border)] hover:bg-[var(--q-card)] hover:shadow-xl md:ml-0 md:w-[45%] md:p-8">
                <h3 className="text-xl font-bold tracking-tight text-[var(--q-text-primary)] md:text-2xl">
                  {item.title}
                </h3>
                <p className="mt-3 text-sm font-medium leading-7 text-secondary-foreground dark:text-[var(--q-text-muted)] md:text-base md:leading-8">
                  {item.description}
                </p>
              </div>
            </div>
