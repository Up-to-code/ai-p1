...
content = content.replace(/import \{ useClientOptionsQuery, useClientQuery, useClientAssetLinksQuery \} from "@\/domains\/clients\/api\/clients";/, 'import { useClientOptionsQuery, useClientQuery } from "@/domains/clients/api/clients";');
content = content.replace(/import \{ useAssetOptionsQuery, useAssetQuery \} from "@\/domains\/assets\/api\/assets";\n/, '');

// 2. Remove asset hooks from CalendarScreen
content = content.replace(/const assetsQuery = useAssetOptionsQuery\(workspaceOrganizationId, \{ enabled: shouldLoadPickerOptions \}\);\n/, '');

// 3. Update BusinessScheduleDialog props to remove assets
...
// 4. Add Big Calendar imports at the top
...
// 6. Delete UntitledCalendarSurface and its child components
const surfaceStartIdx = content.indexOf('function UntitledCalendarSurface({');
if (surfaceStartIdx > -1) {
  content = content.substring(0, surfaceStartIdx);
}

// 7. Add BigCalendarSurface implementation
  ...
  return (
    <section className="h-[calc(100vh-12rem)] min-h-[600px] overflow-hidden rounded-[24px] border border-zinc-200/80 bg-white text-zinc-950 dark:border-white/10 dark:bg-[#0A0A0A] dark:text-white">
      <Calendar
        ...
        onNavigate={(newDate) => {
          // react-big-calendar passes the exact new date. We need to handle this or rely on custom toolbar.
          // For now, let's use the built-in toolbar but pass a custom one later if needed.
          // Since Qentrah had its own toolbar outside the surface, we can hide BigCalendar's toolbar
          // but we already deleted the custom toolbar in calendar-screen.tsx when we deleted UntitledCalendarSurface.
          // Wait! The toolbar was INSIDE UntitledCalendarSurface! We need to bring it back.
        }}
        onView={() => {}}
...
content += bigCalendarImpl;

fs.writeFileSync(file, content);
console.log('Successfully updated calendar-screen.tsx');
