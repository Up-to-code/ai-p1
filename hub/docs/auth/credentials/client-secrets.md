...
Purpose: Client secrets are confidential-client only.

## Owns

- Client secrets are confidential-client only.
- Show once and rotate on request or suspicion.
- Never expose client secrets to browser code.
...
- Keep this file focused on client secrets only.
