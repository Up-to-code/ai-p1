...
import type { ClientType } from "../store/clients.types";
...
export function useClientsPagedQuery(organizationId?: string, options?: { type?: ClientType; search?: string }) {
  ...
  const params = useMemo(() => ({ type, search }), [search, type]);
...
export function useClientOptionsQuery(organizationId?: string) {
  return useHttpQuery<{ id: string; name: string }[]>(
    ["clients-options", organizationId],
    organizationId ? `/api/v1/organizations/${organizationId}/read/clients/options` : undefined,
  );
}
...
export async function linkClientUnitRequest(organizationId: string, clientId: string, propertyId: string, status = "interested") {
