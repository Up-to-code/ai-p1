"use client";

import { Link } from "@/i18n/routing";
import { UserPlus, Gavel, ShieldCheck, ChevronLeft } from "lucide-react";
import { useTranslations } from "next-intl";
import { Section } from "@/components/landing/core/section";
import { PageHero } from "@/components/landing/core/page-hero";
import InstitutionalButton from "@/components/ui/institutional-button";

export default function SignUpPage() {
  const t = useTranslations("signin");

  const trustSignals = [
    { icon: ShieldCheck, label: t("encrypted") },
    { icon: Gavel, label: t("clearTerms") },
  ];

  return (
    <main className="flex min-h-screen flex-col bg-background pt-20 font-sans text-foreground selection:bg-blue-600 selection:text-white transition-colors">
      <div className="absolute top-8 left-8 rtl:left-auto rtl:right-8 z-10">
        <Link 
            href="/" 
            className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors"
        >
            <ChevronLeft className="h-4 w-4 rtl:rotate-180" />
            {t("backToHome")}
        </Link>
      </div>

      <Section className="flex flex-1 items-center justify-center pb-24">
        <div className="w-full max-w-md">
          <PageHero
            contentClassName="space-y-12 text-center"
            badge={(
              <div className="space-y-6">
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-600 shadow-none">
                  <UserPlus className="h-8 w-8 text-white" />
                </div>
              </div>
            )}
            title="Create an account"
            titleTag="h1"
            titleClassName="text-4xl font-black uppercase tracking-tight text-slate-900 dark:text-slate-100"
            description={(
              <p className="font-bold text-slate-500 dark:text-slate-300">
                Join the central synchronization engine for Saudi real estate.
              </p>
            )}
            actions={(
              <>
                <div className="space-y-6">
                  <InstitutionalButton
                    variant="dark"
                    className="flex w-full items-center justify-center gap-4"
                    href="/dashboard"
                  >
                    <svg className="h-5 w-5" viewBox="0 0 24 24" aria-hidden="true">
                        <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                        <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                        <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" />
                        <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                    </svg>
                    Continue with Google
                  </InstitutionalButton>
                  <p className="text-[10px] font-black uppercase tracking-widest leading-relaxed text-slate-400">
                    {t("agreementPrefix")}
                    {" "}
                    <Link href="/terms" className="text-blue-600 hover:underline focus-visible:underline">{t("agreementTerms")}</Link>
                    {" "}{t("agreementAnd")}{" "}
                    <Link href="/policy" className="text-blue-600 hover:underline focus-visible:underline">{t("agreementPrivacy")}</Link>
                    {" "}{t("agreementSuffix")}
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-8 border-t border-slate-100 pt-12 dark:border-slate-800">
                  {trustSignals.map(({ icon: Icon, label }) => (
                    <div key={label} className="flex flex-col items-center gap-3">
                      <Icon className="h-5 w-5 text-slate-300 dark:text-slate-500" />
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">{label}</span>
                    </div>
                  ))}
                </div>
              </>
            )}
          />
        </div>
      </Section>
    </main>
  );
}
