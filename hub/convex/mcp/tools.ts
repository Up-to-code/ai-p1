  ...
  handler: async (ctx, args) => {
    ...
    if (args.tool === "clients_list") {
      const clients = await ctx.db
        ...
        .withIndex("by_organization_id", (q) => q.eq("organizationId", args.organizationId))
        .collect();
      return clients.filter((client) => !client.deletedAt).sort((a, b) => b.updatedAt - a.updatedAt).map(present);
    }

    if (args.tool === "clients_get") {
      const client = await ctx.db.get(requiredString(input, "clientId") as Id<"clients">);
      return present(assertActiveOrganization(client, args.organizationId, "Client"));
    }
    ...
    if (args.tool === "properties_list") {
      const units = await ctx.db
        ...
        .withIndex("by_organization_id", (q) => q.eq("organizationId", args.organizationId))
        .collect();
      return units.filter((unit) => !unit.deletedAt).sort((a, b) => b.updatedAt - a.updatedAt).map(present);
    }
    ...
    if (args.tool === "properties_get" || args.tool === "properties_open") {
      ...
      const result = present(assertActiveOrganization(property, args.organizationId, "Property unit"));
    ...
    if (args.tool === "projects_list") {
      const projects = await ctx.db
        ...
        .withIndex("by_organization_id", (q) => q.eq("organizationId", args.organizationId))
        .collect();
      return projects.filter((project) => !project.deletedAt).sort((a, b) => b.updatedAt - a.updatedAt).map(present);
    }

    if (args.tool === "projects_get") {
      const project = await ctx.db.get(requiredString(input, "projectId") as Id<"projects">);
      return present(assertActiveOrganization(project, args.organizationId, "Project"));
    }
    ...
    if (args.tool === "calendar_list_today") {
      ...
      return listEvents(ctx, args.organizationId, start, end);
    }

    if (args.tool === "calendar_list_range") {
      return listEvents(ctx, args.organizationId, requiredNumber(input, "startAt"), requiredNumber(input, "endAt"));
    }
    ...
    if (args.tool === "tasks_list") {
      ...
      const tasks = clientId
        ...
        : await ctx.db
            ...
            .withIndex("by_organization_id", (q) => q.eq("organizationId", args.organizationId))
            .collect();
      return tasks.filter((task) => !task.deletedAt).sort((a, b) => (a.dueAt ?? 0) - (b.dueAt ?? 0)).map(present);
    }

    if (args.tool === "tasks_get") {
      const task = await ctx.db.get(requiredString(input, "taskId") as Id<"clientTasks">);
      return present(assertActiveOrganization(task, args.organizationId, "Task"));
    }
      ...
      const media = await ctx.db
        ...
        .withIndex("by_organization_resource", (q) =>
          q
            ...
            .eq("resourceType", requiredString(input, "resourceType") as "project" | "property" | "client" | "calendarEvent" | "task")
...
async function listEvents(
  ...
  const events = await ctx.db
    ...
    .collect();
  return events.filter((event) => !event.deletedAt).sort((a, b) => a.startAt - b.startAt).map(present);
}
...
function calendarInput(input: Input) {
  return {
    ...
    type: oneOf(input.type, ["client-visit", "site-viewing", "appointment", "signing", "follow-up", "handover", "audit", "custom"] as const, "appointment"),
