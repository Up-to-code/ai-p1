...
export type WorkspaceAssetCardContentProps = {
  image: string;
  title: string;
  location: string;
  priceLabel: string;
...
/**
 * WHY:   Multiple workspace zones need one reusable asset card instead of bespoke text blocks.
 * WHAT:  Renders a clean, minimal asset card with image, key info, specs, and optional footer.
 * HOW:   Uses a simple visual header with a tighter layout that keeps attention on the item itself.
 */
export function WorkspaceAssetCardContent({
  image,
  title,
  location,
  priceLabel,
  ...
  publicationBadge,
  density = "compact",
}: WorkspaceAssetCardContentProps) {
  return (
    <article
      className={cn(
...
export default memo(WorkspaceAssetCardContent);
