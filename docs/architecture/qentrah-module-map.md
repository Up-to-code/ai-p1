...
| Projects | `src/domains/projects`; Project Workspace codec in `src/domains/projects/workspace` | `convex/projects`, `projectSpaces`; authorized collection surface and tree in `convex/projectWorkspace` | canonical `/projects/{table,list,board,calendar,timeline,dashboard}` routes, contextual sidebar, MCP, Eve | relation access, visibility, one authorized tab projection, transactional tab lifecycle, route-owned view Adapters, Saved View-backed dashboard layout |
...
| Saved Views | shared hooks and contracts in `src/domains/views`; resource-specific UI Adapters in Tasks and Projects | `convex/savedViews`, `savedViewGrants`; access in `convex/access/savedView*.ts`; surface orchestration in owning Modules such as `convex/projectWorkspace` | Task and Project Resource Workspaces | live user/Team grants, parent-scope reauthorization, personal/shared/protected behavior, revision and audit; sharing never grants record access |
