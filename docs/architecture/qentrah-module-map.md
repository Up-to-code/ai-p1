...
| Search | `src/domains/search`; search contracts in `@qentrah/domain-contracts` | `convex/search`, including private saved/recent queries and privacy-minimized analytics | Meilisearch worker, command palette, route-backed Search Center, extraction Adapters | policy-filtered projection, indexed facet fields, outbox idempotency, multilingual ranking, final record reauthorization |
